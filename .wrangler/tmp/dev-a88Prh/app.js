...
// .wrangler/tmp/bundle-XzctGh/strip-cf-connecting-ip-header.js
...
var JWT_SECRET = "super-secret-key-change-this";
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  try {
    const payload = await verify2(token, c.env.JWT_SECRET || JWT_SECRET);
    c.set("user", payload);
    await next();
  } catch (err) {
...
app.post("/auth/login", async (c) => {
  try {
    ...
    const token = await sign2(payload, c.env.JWT_SECRET || JWT_SECRET);
...
app.get("/cats", async (c) => {
  try {
    ...
    const cats = await c.env.DB.prepare("SELECT * FROM cats LIMIT ? OFFSET ?").bind(limit, offset).all();
...
app.get("/cats/:id", async (c) => {
  try {
    ...
    const cat = await c.env.DB.prepare("SELECT * FROM cats WHERE id = ?").bind(id).first();
    if (!cat) {
...
app.delete("/cats/:id", authMiddleware, async (c) => {
  try {
    const id = c.req.param("id");
    const result = await c.env.DB.prepare("DELETE FROM cats WHERE id = ?").bind(id).run();
    if (result.meta.changes === 0) {
      return c.json({ error: "Cat not found" }, 404);
    }
...
// .wrangler/tmp/bundle-XzctGh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XzctGh/middleware-loader.entry.ts
